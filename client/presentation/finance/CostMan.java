...
public class CostMan {
	...
	public void costMan(JPanel context) {
		...
		int gap = 20;
		int gaph = 50;
		JLabel[] label = new JLabel[] { new JLabel("付款日期:"),
				new JLabel("付款金额:"), new JLabel("付款人:"), new JLabel("付款账号:"),
		...
		final JTextField[] textfield = new JTextField[] { new JTextField(),
				new JTextField(), new JTextField(), new JTextField(),
				new JTextField(), new JTextField() };
		for (int i = 1; i < textfield.length - 1; i++) {
			textfield[i].setBounds(200 + gap + labelw, 30 + (texth + gaph) * i,
					textw, texth);
			context.add(textfield[i]);
		}
		textfield[textfield.length - 1].setBounds(200 + gap + labelw, 30
				+ (texth + gaph) * (textfield.length - 1), 150 + textw, texth);
		...
		context.add(yearbox);
		yearbox.setBounds(200 + gap + labelw, 30, 80, texth);
		context.add(monthbox);
		monthbox.setBounds(200 + gap + labelw + 90, 30, 80, texth);
		context.add(daybox);
		daybox.setBounds(200 + gap + labelw + 180, 30, 80, texth);
		...
		okbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < textfield[1].getText().length(); i++) {
					if (textfield[1].getText().charAt(i) > '9'
							|| textfield[1].getText().charAt(i) < '1') {
						isnum = false;
					}
				}
				boolean isid = true;
				...
				boolean isempty = priceisempty || payerisempty || idisempty
						|| infoisempty || tipsisempty;
