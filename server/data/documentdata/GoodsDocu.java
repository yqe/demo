...
public GoodsDocuPO find(String ID) throws RemoteException {
	...
	try {
		...
		String find="SELECT 寄件人姓名,寄件人住址,寄件人单位,寄件人手机,收件人姓名,收件人住址,收件人单位"
				+ "，收件人手机,包装费,费用合计,快递类型,收件日期,货物名称,货物数量,货物重量,长,宽,高,体积,货物信息,包装选择,预计到达时间,寄件单生成日期,快递员姓名"+" FROM 快递单"+" WHERE 订单条形码号='"+goodsID+"'";
		...
		this.goodsname=rs.getString(12);
		this.goodsnumber=rs.getInt(13);
		this.length=rs.getDouble(14);
		this.width=rs.getDouble(15);
		this.height=rs.getDouble(16);
		this.v=rs.getDouble(17);
		this.goodsinfo=rs.getString(18);
		this.wrappedtype=rs.getString(19);
		this.expectedtime=rs.getString(20);
		this.generatetime=rs.getString(21);
		...
		GoodsDocuPO goodsdocuPO=new GoodsDocuPO(dilivername,diliveraddress,diliverworkspace,
			...
			totalfee,expresstype,goodsID,receivedtime,goodsname,goodsnumber,length,width,height,v,goodsinfo,wrappedtype,expectedtime,generatetime,courier);
		return goodsdocuPO;
...
public void insert(GoodsDocuPO po) throws RemoteException {
	...
	String insert="INSERT INTO 快递单"+" (寄件人姓名,寄件人地址,寄件人单位,寄件人电话,寄件人手机,收件人姓名,收件人地址,收件人单位,收件人电话,收件人手机,"
			+ "包装费,合计费用,快递类型,订单条形码号,收件日期,货物名称,货物数量,货物重量,长,宽,高,体积,货物信息,包装选择,预计到达时间,寄件单生成日期,快递员姓名)"+" VALUES('"+dilivername+"','"+diliveraddress+"','"+diliverworkspace+"','"+diliverhomephone+"','"+receivername+"','"+receiveraddress+"','"+receiverworkspace+"','"+receiverhomephone+"','"+receivermobile+"',"+wrappedfee+","+totalfee+",'"+expresstype+"','"+goodsID+"','"+receivedtime+"',"
