...
public class TransData implements TransDataService {
	...
	public TransPO find(String carsID) throws RemoteException {
		...
		try {
			...
			TransPO trapo = new TransPO(date, bussinessID, expreID, destination, carsID, monitor, supercargo, fee,
					orderID);
			rs.close();
			return trapo;
	...
	public boolean insert(TransPO po) {
		...
		try {
			...
			String insert = "INSERT INTO 装车单" + " (装车日期,本营业厅编号,汽运编号,到达地,车辆代号,监装员,押运员,运费,装箱中订单条形码号)" + " VALUES('" + date
					+ "','" + bussinessID + "','" + expreID + "','" + destination + "','" + carsID + "','" + monitor
					+ "','" + supercargo + "'," + fee + ",'" + orderID + "')";
	...
	public ArrayList<TransPO> findmore() {
		...
		try {
			...
			while (rs.next()) {
				transList.add(new TransPO(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4),
			...
			errtransList.add(
					new TransPO("不存在", bussinessID, expreID, destination, carsID, monitor, supercargo, fee, orderID));
			return errtransList;
		}
