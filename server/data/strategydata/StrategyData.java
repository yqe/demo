...
public class StrategyData implements StrategyDataService {
	...
	public StrategyPO observe() throws RemoteException {
		...
		try {
			...
			rs.next();
			StrategyPO strapo = new StrategyPO(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getInt(4), rs.getInt(5),
	...
	public boolean updatesalary(StrategyPO po) throws RemoteException {
		...
		try {
			...
			String insert = "INSERT INTO 经营策略" + " (总经理,营业厅业务员,中转中心业务员,中转中心库存管理人员,快递员,财务人员,管理员,快递费价格计算常量)" + " VALUES("
					+ topsal + "," + busssal + "," + storagemanagersal + "," + storagesal + "," + financesal + ","
					+ diliversal + "," + managersal + "," + constance + ")";
